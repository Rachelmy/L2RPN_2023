...
"""
Originally from:
https://github.com/rte-france/l2rpn-baselines/blob/master/l2rpn_baselines/OptimCVXPY/optimCVXPY.py
Modified by:
...
def load_action_to_grid2op(n_line, action_space, action_vec_path, action_threshold=None, return_counts=False):
    ...
    for i in range(n_line):
        full_path = '%s/line_%d_act.npz'%(action_vec_path,i)
        assert full_path.endswith('.npz') and os.path.exists(full_path), FileNotFoundError(
            f'file_paths {full_path} does not contain a valid npz file.')
        data = np.load(full_path, allow_pickle=True)
        act = data['action_space']
        cnt = data['counts']
        # keep em sorted
        print(act.shape)
        act = [x for _,x in sorted(zip(cnt,act),key=lambda x: x[0])]
        act = [convert_from_vect(action_space,x) for x in act]
        cnt = sorted(cnt)
        all_actions.append(act)
        counts.append(cnt)
    global_pool_actions = []
    for actx in all_actions:
        global_pool_actions += actx
    global_pool_counts = np.concatenate(counts,axis=0)
    global_pool_actions = [x for _,x in sorted(zip(global_pool_counts,global_pool_actions),key=lambda x: x[0])]
    global_pool_counts = sorted(global_pool_counts)
    all_actions.append(global_pool_actions)
    counts.append(global_pool_counts)
    # no have a 'global' action that collects all em
...
class OptimCVXPY(BaseAgent):
    ...
    def _load_topo_actions(self, action_space_path):
        ...
        self.topo_actions_to_check = load_action_to_grid2op(
            self.n_line,self.action_space, action_space_path)
        self.num_topo_actions_to_check = 344 # no cap aint arguin wit dis number
    ...
    def change_substation_topology(self, observation, base_action, min_rho=None):
        ...
        ## code here to dynamically load lines
        
        add_actions = 30
        lines_overloaded = np.arange(observation.n_line)[observation.rho>self.rho_danger]
        action_buf = []
        for line in lines_overloaded:
            action_buf += self.topo_actions_to_check[line][:int((1/len(lines_overloaded))*self.num_topo_actions_to_check)]
            # anyways add some actions
            action_buf += self.topo_actions_to_check[observation.n_line][:add_actions]
        if len(action_buf) < self.num_topo_actions_to_check:
            def_w = int(self.num_topo_actions_to_check-len(action_buf))
            action_buf += self.topo_actions_to_check[observation.n_line][:def_w]
            if def_w < add_actions:
                action_buf += self.topo_actions_to_check[observation.n_line][def_w:add_actions]
        
        
        for action in action_buf:
